...
int main(int argc, char *argv[])
{
    ...
    int column = 5;
        ...
        //Check if cmd line has a 2nd argument
    if (argc > 1) {
        //If so, take argument as tolerance value
        double tval = atof(argv[1]);
        //cout << tval << endl;
        ...
        for (int i = 0; i < row; i++) {
            ...
            float h = testdata[i*column+0];
            float wc = testdata[i*column+1];
            float bsa = testdata[i*column+2];
            float vtc = testdata[i*column + 3];
            float sbsi = testdata[i*column + 4];
            //SBSI val = bodyShapeIndex(&sbsi);
